---
description: Loads the VisaFusion SpecKit constitution (SDD governance). Use whenever starting a work item, module, or when asked to initialize/apply SpecKit standards.
agent: visafusion
---

# VisaFusion SpecKit Constitution

You are now operating under the VisaFusion SpecKit constitution. Apply it to
every subsequent engineering activity in this session. Read `@library/03_SpecKit_SDD_Framework.md`
and `@library/12_VisaFusion_Legacy_Modernization_Playbook.md` before acting.
...
1. If the `/specs` folder tree does not yet exist, initialize it exactly as
   specified in `@library/03_SpecKit_SDD_Framework.md` §3:
   `/specs/epics`, `/specs/features`, `/specs/modules`, `/specs/database`,
   `/specs/api`, `/specs/ui`, `/specs/security`, `/specs/migration`,
   `/specs/testing`.
2. For every work item, before writing any code:
   ...
   - Create an ADR (`ADR-XXXX`) for every architectural decision, stored under
     `/adr`.
   - Map the work item to its legacy pages using `@findings/modernization_plan.md`
     §6 (module map) and §13 (legacy pages).
