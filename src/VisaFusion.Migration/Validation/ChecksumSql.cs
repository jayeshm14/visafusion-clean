...
public static class ChecksumSql
{
        ...
        CancellationToken ct = default)
    {
        ...
        cmd.CommandText = sql;
        // The checksum is a full-table SHA2_256 scan; over the verified data
        // volumes (Mainentry 271k+, PaxStatus 359k+) it can exceed the 30s
        ...
        var sql = @"
            SELECT c.COLUMN_NAME, c.DATA_TYPE
             ...
             WHERE c.TABLE_NAME = @table
               ...
               -- Additive modernization columns (SPEC-0006 §16, AC-011): the
               -- rowversion/timestamp token on dbo.Mainentry (added by script
               -- 09_add_entry_rowversion.sql) is value-generated by SQL Server
               -- and holds NO copied legacy data, so including it would make the
               -- source/target checksums diverge on schema alone. Excluded the
               -- same way identity columns are (see AuditTableTests FR-009 note).
               AND c.DATA_TYPE NOT IN ('rowversion', 'timestamp')
