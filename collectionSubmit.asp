...
for k=1 to entries
	...
	response.write "<tr> <td colspan=6><font size=2 color='#0000ff'><b>"&ucase(Temppaxname)&"</b></font></td></tr>"
		...
		'response.Write "<br> Country: " &country & "subdate"&k &Request.Form("subdate"&k)& "coldate"&k &Request.Form("coldate"&k)
				...
				End if
					if Tempsubdate<>"" and not isnull(Tempsubdate) then
...
while i<= ivalue 
rs.open "select * from  paxstatus where refno ="&refno &" and paxID="& request("paxid"&i) &" and countryID="&request("countryinv"&i),con,2,3
if  rs.eof then
i=i+1
rs.close
else
