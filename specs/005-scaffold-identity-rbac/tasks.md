...
- [x] T040 [P] [US1] Create Web `/Auth/Register` Razor Page (guest-only registration form; role fixed server-side to `guest`; success mirrors `regsubdone.asp`; errors rendered inline from the API problem-details) in `src/VisaFusion.Web/Pages/Auth/Register.cshtml` + `.cs` (spec §14, contracts/web-ui.md §1.3, plan.md line 197) — delivered 2026-08-13 (review finding: the page was documented in the contract/plan but never task-ified, so the US5 `regsub*.asp` → `/Auth/Register` 301 ended in a 404; the register rules were extracted into the shared `RegistrationFlow` (`src/VisaFusion.Api/Registration/RegistrationFlow.cs`) used by both the `POST /api/v1/public/register` endpoint and the page — the page calls it in-process instead of via HttpClient, because a raw HTTP round-trip cannot route through the TestServer in functional tests and would otherwise have duplicated the endpoint logic; `tests/FunctionalTests/RegisterPageTests.cs` (5 tests, antiforgery via the factory client's cookie jar) + rewrite functional test strengthened to follow the redirect target → 200)
...
- [x] T016 [P] [US2] Unit tests for the day-gate evaluation (open day / no row / closed row → `rsn=O`; `rsn=C` never produced) in `tests/UnitTests` (AC-011/TS-013, FR-018) — delivered 2026-08-13 (`tests/UnitTests/SecurityGateServiceTests.cs`, 7 tests, all green; EF InMemory test-only package — deviation log §6)
- [x] T017 [P] [US2] Integration test seeding the `security` table: no row today / row with closing time set / open row → `403 rsn=O` / `403 rsn=O` / success (TS-013; self-skipping when SQL Server is unreachable) in `tests/IntegrationTests` — delivered 2026-08-13 (`tests/IntegrationTests/SecurityGateIntegrationTests.cs`; seeds a synthetic date — never today — and deletes test rows in `finally`; self-skips when SQL Server is unreachable or the `security` table is absent)

### Implementation for User Story 2

- [x] T018 [US2] Implement the day-gate evaluation on `ISecurityGateService`/`SecurityGateService` (single-source business rule shared by Web + API; reads `SecurityDay` via `VisaEntryDbContext` — read-only, `security` table; FR-018, data-model.md §2.1) — delivered 2026-08-13. DEVIATION (approved, deviation log §5): the interface + `SecurityGateDecision` live in `src/VisaFusion.Core/Application/SecurityGateService.cs`; the implementation lives in `src/VisaFusion.Data/Application/SecurityGateService.cs` (Core cannot reference Data — one-way Data → Core); DI registration moved from `CoreServiceCollectionExtensions` to `VisaFusion.Web/Program.cs` (composition root)
- [x] T019 [US2] Wire the day-gate into the API login (`emp` only; rejected → 403 with `{ rsn: "O" }` problem-details body) in `src/VisaFusion.Api/Endpoints/AuthEndpoint.cs` (depends on T018, T011) — delivered 2026-08-13 (gate evaluated after credential validation, mirroring `authenticate.asp` lines 62–79; `rsn` extension on the problem-details body)
- [x] T020 [US2] Wire the day-gate into the Web cookie login (`emp` only; rejected → redirect `/Auth/Login?rsn=O`) in `src/VisaFusion.Web/Pages/Auth/Login.cshtml.cs` (depends on T018, T013) — delivered 2026-08-13 (credentials validated first, then gate, then `SignInAsync` — a rejected emp login never receives a cookie)
...
- [x] T021 [P] [US3] Functional test for change-password outcomes (valid → 204 + stored hash, no lowercase/plaintext; wrong current / new≠confirm / under 8 chars → 400) in `tests/FunctionalTests` (AC-012/TS-014) — delivered 2026-08-13 (`tests/FunctionalTests/ChangePasswordTests.cs`, 5 tests: 204 + re-auth with new password + old fails; wrong current → 400; new≠confirm → 400; under 8 → 400; unauthenticated → 401)

### Implementation for User Story 3

- [x] T022 [P] [US3] Create Web `/Auth/ChangePassword` Razor Page (inline success / mismatch / policy-violation messages mirroring `changepassword.asp?flag=1|2|3`) in `src/VisaFusion.Web/Pages/Auth/ChangePassword.cshtml` + `.cs` (spec §14, contracts/web-ui.md §1.4) — delivered 2026-08-13 (`ChangePassword.cshtml` + `ChangePassword.cshtml.cs`, `[Authorize]`, inline messages mirroring legacy flags 1/2/3)
- [x] T023 [US3] Implement `POST /api/v1/auth/change-password` (current-password verified via `UserManager`, new password policy-validated, stored via `ChangePasswordAsync`, 204/400) in `src/VisaFusion.Api/Endpoints/AuthEndpoint.cs` (depends on T022; FR-019, contracts/auth-api.md §3) — delivered 2026-08-13 (`AuthEndpoint.ChangePasswordAsync`: user resolved via `FindByNameAsync(Identity?.Name)` because JWT `sub` maps to nameidentifier; required-field/confirm/policy checks → 400; `CheckPasswordAsync` → 400; `ChangePasswordAsync` → 204; route registered with JwtBearer scheme, any role)
...
- [x] T024 [P] [US4] Unit tests for the policy catalog — each of the 11 policies maps to the correct §4.2 role set (EntryOperations, AgentSelf, AgentLedger, BillingOperations, Search, UserManagement, HolidayAdmin, SecurityGate, PasswordSelf, AdminPanel, SuperUserOnly) in `tests/UnitTests` — delivered 2026-08-13 (`tests/UnitTests/AuthorizationPoliciesTests.cs`, 12 tests: Theory over the 10 role-based policies, SuperUserOnly claim requirement, RoleSets exactly 10 entries; RED first — compile error, then GREEN after the catalog landed)
- [x] T025 [P] [US4] Functional test for the 13 §4.3 endpoints matrix — anonymous → 401, wrong role → 403, correct role → 501 (or success for the 2 public routes) in `tests/FunctionalTests` (AC-004/TS-004; token-minted per role) — delivered 2026-08-13 (`tests/FunctionalTests/SecuredWriteRoutesTests.cs`, 38 cases: 11 secured routes × 401/403/501 + public register 201 + public queries 501; token minting helper with optional AgentId claim)
- [x] T026 [P] [US4] Functional test: registration escalation attempt rejected (privileged role in payload ignored → `guest`), registration password under 8 chars rejected (AC-005/TS-005) in `tests/FunctionalTests` — delivered 2026-08-13 (`tests/FunctionalTests/RegistrationEscalationTests.cs`: role=su in payload ignored → guest-only login; under-8 password → 400)
- [x] T027 [P] [US4] Functional tests: backdoor query params (`udaanappraj123guruadm`, `udaan12345functiondisplaymarquee`) have no effect on any route (AC-006/TS-006); agent isolation — agent A requesting agent B's id → 403 (AC-003/TS-003) in `tests/FunctionalTests` — delivered 2026-08-13 (`tests/FunctionalTests/BackdoorAndIsolationTests.cs`: byte-identical health responses; identical login outcome on problem-details title/status — raw bytes differ only in the per-request traceId; agent A id 999 → 403, own id 5771 → 501)

### Implementation for User Story 4

- [x] T028 [P] [US4] Create the authorization policy catalog (11 policies derived from the §4.2 module × role matrix) in `src/VisaFusion.Api/Authorization/AuthorizationPolicies.cs` (spec §5, FR-010) — delivered 2026-08-13 (11 policy consts; `RoleSets` dict of 10 role-based policies verbatim from the §4.2 cells — UserManagement resolved to `adm,su` (its "adm/emp only" row note contradicts its own emp=❌ cell); SuperUserOnly claim-based `RequireClaim(SuperUserClaimType, "true")` per plan.md §4.1/§10; `Register(AuthorizationOptions)` extension)
- [x] T029 [P] [US4] Create the shared `SecuredPlaceholderEndpoint` handler (auth + role + 501 Not Implemented for routes whose module feature has not landed) in `src/VisaFusion.Api/Authorization/SecuredPlaceholderEndpoint.cs` (spec §15, plan.md §Constraints) — delivered 2026-08-13 (`Handle` → 501 problem-details; `HandleOwnAgent(ctx, id)` → 403 on missing/mismatched `AgentId` claim per FR-016, else 501; `WriteProblemAsync` via `ApiError`)
- [x] T030 [US4] Register the 11 role-secured §4.3 write routes with named target routes + minimum roles (501 until module feature) and the 2 public-by-design routes (anonymous, validated, rate-limited per §17/R7 — configuration-driven only) — `POST /api/v1/entries/{refno}/status`, `POST /api/v1/billing/entries`, `DELETE /api/v1/holidays/{id}`, `POST/DELETE /api/v1/holidays`, `POST /api/v1/entries/{refno}/awb`, `POST /api/v1/reports/agent-status/today`, `POST /api/v1/admin/security-day/open`, `POST /api/v1/admin/security-day/close`, `POST /api/v1/entries`, `PUT /api/v1/agents/{id}`, `PUT /api/v1/agents/{id}/self`, `POST /api/v1/public/queries` — registered in `src/VisaFusion.Web/Program.cs` following the existing `app.MapGet("/api/v1/...")` pattern (handlers/501 placeholder in `src/VisaFusion.Api/`; depends on T028, T029; sequential with T009/T031 — shared Program.cs; FR-011/012) — delivered 2026-08-13 (all 11 secured routes registered with inline `Roles` verbatim from §4.3 + JwtBearer scheme + `{id:int}` constraints; `/self` → `HandleOwnAgent`; `public/register` anonymous 201; `public/queries` anonymous 501 with conditional config-driven `RequireRateLimiting("queries")`; register limiter preserved; `out` vars pre-declared to avoid CS0165 on `&&` short-circuit)
- [x] T031 [US4] Apply the policy catalog to the existing representative endpoints, replacing inline role lists, and enforce claim-bound `AgentId` scoping on agent status/statement reads (FR-010/016) in `src/VisaFusion.Api/Endpoints/` (registrations updated in `src/VisaFusion.Web/Program.cs`; depends on T028; sequential with T030 — shared Program.cs), and log authorization denials (subject, endpoint, outcome) without any password material per spec §19/NFR-006 — delivered 2026-08-13 (`AddAuthorization(AuthorizationPolicies.Register)`; representative endpoints switched to policies — employee→EntryOperations, agent→AgentSelf, admin→AdminPanel, billing→BillingOperations; reporting/notifications kept inline `emp,adm,su` with comment — no §4.2 row exists for them; JwtBearer `OnChallenge`/`OnForbidden` write problem-details 401/403 and call `LogAuthorizationDenial(subject, endpoint, outcome)` — no password material)
...
- [x] T032 [P] [US5] Unit tests for the rewrite mapping table (known entry URLs → target routes; unknown/ambiguous `.asp` → 404) in `tests/UnitTests` (AC-007/TS-007) — delivered 2026-08-13 (`tests/UnitTests/LegacyUrlRewriteTests.cs`, 16 tests: 8 known-entry mappings incl. case-insensitivity + subdirectory, 2 unknown-`.asp` handled-with-no-target, 6 non-`.asp` pass-through; RED first — compile error, then GREEN)
- [x] T033 [P] [US5] Functional test: bookmarked legacy `.asp` URLs resolve; unknown legacy URLs → 404 (AC-007/TS-007) in `tests/FunctionalTests` — delivered 2026-08-13 (`tests/FunctionalTests/LegacyUrlRewriteTests.cs`, 9 tests: 6 legacy entry URLs → 301 + exact Location, 2 unknown `.asp` → 404, modern `/api/v1/health` unaffected; client created with `AllowAutoRedirect = false`)

### Implementation for User Story 5

- [x] T034 [US5] Implement `LegacyUrlRewriteMiddleware` (301 redirects for the four documented entry URL patterns; 404 for other `.asp` URLs; no extra round trip) in `src/VisaFusion.Web/Middleware/LegacyUrlRewriteMiddleware.cs` (FR-003, NFR-005) — delivered 2026-08-13 (pure static `Resolve(path)` returning `(bool Handled, string? Target)`; case-insensitive matching; query string preserved on redirect; 404 via `TypedResults.NotFound`)
- [x] T035 [US5] Register the rewrite middleware in the pipeline in `src/VisaFusion.Web/Program.cs` (depends on T034; sequential with T009 — same file) — delivered 2026-08-13 (`app.UseMiddleware<LegacyUrlRewriteMiddleware>()` after `UseSerilogRequestLogging`, before `UseHttpsRedirection` so the redirect Location is built from the normalized scheme)
...
- [x] T036 [P] Run the Phase 0 end-to-end verification per `quickstart.md`: build + boot + 5-role login, static assets self-hosted (AC-008/TS-008), SQL-injection regression `'` inputs stay parameterized (TS-011), golden-file parity where applicable (TS-012), identity import re-run is a no-op (AC-009/TS-009) — complements T007 (the functional 5-role login test); this task is the post-implementation end-to-end validation run — delivered 2026-08-13 (`tests/FunctionalTests/Phase0E2ETests.cs`, 10 tests: 6 static-asset directories → 200; 3 SQLi login payloads → 401; SQLi-shaped username rejected by Identity username validation → 400 (never reaches a query); TS-009 covered by `tests/IntegrationTests/IdentityImportTests.cs` (self-skip w/o SQL Server); TS-012 parity evidenced by AuthLoginTests + ChangePasswordTests; full suite green: Unit 106/106, Functional 92/92, build 0 warnings/0 errors)
- [x] T037 [P] Security spot-checks per `quickstart.md` §5: no plaintext password material in logs/responses/config, backdoor params inert, `ProductionSecretsGuard` fails fast in Production — delivered 2026-08-13 (`tests/FunctionalTests/SecuritySpotCheckTests.cs`, 3 tests: no password material in any response surface (register 201 empty body, login body has no password/`Password` field, change-password 204 empty body); deferred admin/agent-password routes 404 per secured-write-routes.md §3; no password placeholder in any `Log*` template verified across src/; backdoor inertness from T027; `ProductionSecretsGuard` fail-fast from `tests/UnitTests/ProductionSecretsGuardTests.cs`)
- [x] T038 [P] Update the knowledge graph (`knowledge-graph/kg.json` + `knowledge-graph/traceability-matrix.md`) with SPEC-0005 completion — FR/AC/TS → components/endpoints/tests per spec §24 — and update the decision log (no new ADR required: ADR-0001 baseline + the two 2026-08-11 clarification decisions are already recorded); per the constitution Mission (knowledge graph updated after every completed task), sync `kg.json` at each completed implementation milestone during the run, not only at the end — delivered 2026-08-13 (kg.json now 73 nodes / 108 edges: SPEC-0005 + AUTH-API + AUTH-REGISTER + SEC-RBAC-POLICIES + SEC-ROUTES + SEC-BACKDOOR-FIXED + URL-REWRITE + UI-LOGIN/REGISTER/CHANGEPASSWORD + TEST-AUTH with 22 edges; traceability-matrix.md gained the SPEC-0005 section: FR-001..FR-019/NFR rows + TS-001..TS-014 → test-artifact map + module mapping; no new ADR needed)
- [x] T039 [P] Documentation updates: this feature's docs (contracts remain current), solution-level release notes for Phase 0, and the spec traceability matrix cross-check (constitution Principle IV/V) — delivered 2026-08-13 (contracts/auth-api.md, secured-write-routes.md, web-ui.md verified current against the implementation — no deviations; `reports/release-notes/phase-0.md` created with Phase 0 delivery summary, test status, known limitations, and exit-criterion evidence; traceability matrix cross-checked in T038)
