...
SentDate=usrtosysdate(request("date"))
set rsSentMail=server.createobject("adodb.recordset")
stmtSentMail="select distinct(agentsid) from sentmails where day(date) = "&day(SentDate)&" and month(date)="&month(SentDate)&" and year(date)="&year(SentDate)&" and toemail <> '' order by agentsid"
rsSentMail.open stmtSentMail,con,3,3

if not rsSentMail.EOF then
	while not rsSentMail.eof

	if trim(request("awb"))="" then
		set rsawb=server.createObject("ADODB.recordset")
		rsawb.open "select * from sentawb where agentsid="&rsSentMail("agentsid")&" and date='"&today-1&"'",con
		awbstr=""
	...
	agentAddress=getAgentAddress(rsSentMail("agentsid"))
...
EmailBody=EmailBody& "</tr>"
	...
	EmailBody=EmailBody &"<tr><td><font size=""2"" color=""#FFFFFF"" face=""verdana"">SENT BY : <b> Udaan India Pvt. Ltd</b> </font>"
	...
	EmailBody=EmailBody &"<b>Sales & Marketing :</b> 01166603653/9873355648   <b> HRD & Admin :</b> 01126182402/03 Ext:114 <br>      <b> Accounts :</b> 01166603652/9818119166 <br>      <b>Udaan PRO :</b> 01126103864/9313368934 <br>"
	...
	rsAgentEmail.open "select * from Agents where agentsid="&rsSentMail("agentsid"),con
	...
	If agentEmail<>"" and agentEmail<>" " then
...
agentEmail = ""
		...
		emailBody=""
		'Response.write "<br><p align=center> Email has been sent to the agent at <b> "&agentEmail&"</b></p>"
	end if
rsSentMail.movenext
wend
end if
