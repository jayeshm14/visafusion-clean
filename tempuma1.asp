...
stmt="select * from Mainentry where refno='"&77990&"'"
...
<%
function weeklyHolidayList(currentdate, countryid)
set rsw=server.createobject("adodb.recordset")
set rsCountry=server.createobject("adodb.recordset")
today=currentdate
cid=countryid
tillday=today+360
stmt ="select * from holidaylist where holiday between '"&today&"' and '"&tillday&"' and countryid in ("&cid&") ORDER BY HOLIDAY"
rs.open stmt,con
if rs.eof then 
Display="" 
else 
...
rs.close()
End if
weeklyHolidayList=ucase(Display)
End function

IF weeklyHolidayList(date(), countryid)<>"" THEN
%>
  <tr bgcolor="#F0F0FF" valign="top"> 
    <td><img src="images/alert1.gif" width="40" height="20"></td>
    <td colspan="14"><B>HOLIDAYS OF NEXT 8 DAYS : <%=weeklyHolidayList(date(), countryid)%></B></td>
  </tr>
<%
END IF
