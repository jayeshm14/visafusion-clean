...
function RemarkPaste(id)
{
  document.collection.remark11.value=document.collection.remarkl11.value;
}
</SCRIPT>
                ...
                <tr> 
                  ...
                  <td width="80%" colspan=3> <font size="2" color="#006600"><b> 
                    <input type="text" name="remark<%=i&l%>" value="<%= ucase(rsinvoice.fields("remarks")) %>" size="70" ><br>
		                      <select name="remarkl<%=i&l%>" size=""1"" onchange="RemarkPaste('<%=i&l%>')">"
	  <% Call LoadListBox("status",rsinvoice.fields("statusID")) %>
                                          
		</SELECT><%=rsinvoice.fields("statusID")%>
                  </td>
                </tr>
                <tr > 
