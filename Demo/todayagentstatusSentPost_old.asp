...
stmtSentMail="select distinct(agentsid) from sentmails where day(date) = "&day(SentDate)&" and month(date)="&month(SentDate)&" and year(date)="&year(SentDate)&" order by agentsid"
...
if not rsSentMail.EOF then
	...
	if rs.eof then 
		...
		if showFlag="Y" then
			...
			if trim(externalremarks)<>"" and refno<>checkrefno then
				remarksforClient=remarksforClient&"<b><font size=""4"">Ref #"&refno&" : </font></b><font size=""3"">"&externalremarks&"</font><br>"
			end if
	...
	if awbstr <> "" Then 
		EmailBody=EmailBody& "<tr><td VALIGN=""TOP"" colspan=""5"" ><BR><font size=""4"" color=""RED""><b><CENTER>TODAY'S AWB NUMBER : <br>"&  awbstr & "</CENTER></b></font></td></tr>"
	End If
	...
	EmailBody=EmailBody &"SENT BY : <b>Udaan India Pvt. Ltd.</b>"
