...
<form name=collection action="collectionform.asp" method="post">
 ...
 stmt="select * from Mainentry  where subdate>"&date1 &" order by agent"
 end if
 if cmd="date" then
 
 stmt="select * from Mainentry  where subdate>"&date1 &" order by receivedate desc"
 end if
 if cmd="today" then
 ...
 if cmd="" or isNull(cmd) then
  ...
  stmt="select * from Mainentry where subdate>"&date1 &" order by refno desc"
 end if
 if cmd="status" then

 stmt="select * from Mainentry  where subdate>"&date1 &" order by Status"
 end if
 if cmd="notsent" then
...
End if
response.write "<tr bgcolor='#F0F0FF'><td><font face='arial' size=2 color='#000000'>"&refno&"</font></td><td><font face='arial' size=2 color='#000000'>"& recdate &"</font></td><td><font face='arial' size=2 color='#000000'>"& subdate &"</font></td><td><font face='arial' size=2 color='#000000'>"& coldate &"</font></td><td><font face='arial' size=2 color='#000000'><a href='collectionform.asp?refno="&refno&"' >"&ucase(rs.fields("paxname"))&"</a></font></td><td><font face='arial' size=2 color='#000000'>"&rs.fields("totalpassengers")&"</font></td><td><font face='arial' size=2 color='#000000'><a href=searchPax.asp?agent="& agent&" >"
