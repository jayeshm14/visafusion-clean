...
certcount=cint(Request.Form ("certcount"))
...
'rs("EntryDateTime")=todaydate

rs("EntryDateTime")=FormatDateTime(now(),0)

rs("AgentInstruction")=request("retrieveremark")
rs("bill")="N"
rs.update
rs.close
...
rs.open "select * from entrydetails where refno=1",con,2,3
	...
	for l=1 to count
		...
		if request.form("country"&k&l)<> "" then
				...
				rsCountry("Refno")=cint(Refno)
				rsCountry("entrytype")=0
				...
				'INSERTING THE VALUES IN TO PAXATTESTATION TABLE
				
						for mm=1 to certcount
						
						if Request.Form("CertificateID"&k&l&mm)<>"" then
						rsAttest.open "Select * from PaxAttestation where paxid=1",con,2,3
						rsAttest.addNew
						rsAttest("PaxID")=cdbl(paxid)
						rsAttest("CountryID")=Cint(Request.Form("country"&k&l))
						rsAttest("AttestationID")=Cint(Request.Form("AttestationID"&k&l&mm))
						rsAttest("CertificateID")=Cint(Request.Form("CertificateID"&k&l&mm))
						rsAttest.update
						rsAttest.close
						end if
						next
...
application.unlock()
response.clear
myurl= "entryAttestation.asp?msgID=1&pname="&request("name1")&"&refno="&refno
