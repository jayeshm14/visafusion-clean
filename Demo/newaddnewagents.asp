...
stmt="select * from newagents where description='"&lcase(request("agentname"))&"'"
...
rs.update
rs.close
rs.open "select max(newagentsid) from newagents",con
if not rs.eof then
agentID=rs(0)
end if
rs.close
...
response.clear
  myurl= "newagenthome.asp?agent="&ucase(request("agentname"))&"&flag=3"
  response.redirect(myurl)
else
response.clear
  myurl= "newagenthome.asp?agent="&ucase(request("agentname"))&"&flag=4"
  response.redirect(myurl)
%>
<table><tr><td>
