...
days=Request("days")
if days<>"" then
date1=Date()-cint(days)
else
date1=Date()
end if
Response.write "<B>EMAIL HAS BEEN SENT TO AGENT FOLLOWING ADDRESS:<br>"
...
mainStmt="select * from mainEntry where status='Pending' and (Day(subdate)>="&day(date1)&" and month(subdate)>="&month(date1)&" and year(subdate)="&year(date1)&")"
...
rsAgentEmail.close()
emailSubject="Udaan -Regarding "&pname & "Status="&status
emailBody=""
...
Wend 
 %>
    ...
    <p align="center"> <A href="collection.asp">CLICK HERE TO GO TO COLLECTION PAGE </A></P>           
                 </td>
              </tr>
            </table>
          </td>
