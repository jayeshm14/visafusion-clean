...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| G | anonymous | Public | Home | — | `/Public/Index` | `Areas/Public/Pages/Index.cshtml` | Public landing (MOD-007) | plain HTML (no `vf-*`) | `views/index.pug` dashboard pattern; Cards `COMPONENT_CATALOG §3.1` | `PublicLanding` | SPEC-0007 | AreaTests, Phase0E2ETests | PARTIAL — static, no page model |
| G | anonymous | Public | Visa Info | — | `/Public/VisaInfo` | `Areas/Public/Pages/VisaInfo.cshtml` | Visa information | plain HTML (no `vf-*`) | Cards §3.1, Accordion/Collapse §5.2, Icons §8 (`cif-*` flags) | `InfoPage` | SPEC-0007 | AreaTests, Phase0E2ETests | PARTIAL — static |
| G | anonymous | Public | Embassy | — | `/Public/Embassy` | `Areas/Public/Pages/Embassy.cshtml` | Embassy directory | plain HTML (no `vf-*`) | Cards §3.1, Tables §3.2, Icons §8 (`cif-*`) | `InfoPage` | SPEC-0007 | AreaTests, Phase0E2ETests | PARTIAL — static |
| G | anonymous | Public | Country Info | — | `/Public/CountryInfo` | `Areas/Public/Pages/CountryInfo.cshtml` | Country information | plain HTML (no `vf-*`) | Cards §3.1, Chip §5.4 | `InfoPage` | SPEC-0007 | AreaTests, Phase0E2ETests | PARTIAL — static |
...
| G | anonymous | Public | Contact | — | `/Public/Contact` | `Areas/Public/Pages/Contact.cshtml` | Contact (legacy `contact.asp`, MOD-007) | plain HTML (no `vf-*`) | Form Control §6.1, Validation §6.5, Buttons §5.1 | `InfoPage` | SPEC-0007 | AreaTests, Phase0E2ETests | PARTIAL — static |
| G | anonymous | Public | Subscribe | — | `/Public/Subscribe` | `Areas/Public/Pages/Subscribe.cshtml` | Newsletter subscription | plain HTML (no `vf-*`) | Form Control §6.1, Input Group §6.2, Buttons §5.1 | `InfoPage` | SPEC-0007 | AreaTests, Phase0E2ETests | PARTIAL — static |
| G | anonymous | Public | Register | — | `/Public/Register` | `Areas/Public/Pages/Register.cshtml` | Public register (legacy `register.asp`, MOD-007) | plain HTML (no `vf-*`) | `authentication/register.pug` (INVENTORY §10) | `AuthCard` | SPEC-0007 | AreaTests, Phase0E2ETests | PARTIAL — static; active registration at `/Auth/Register` |
...
| ADM/SU | (none — placeholder) | Notifications | Home | — | `/Notifications/Index` | `Areas/Notifications/Pages/Index.cshtml` | Notifications (MOD-004 `SendSMS.asp`; SPEC-0008 notifications-api; dispatch log-only, GAP-008) | plain HTML | `views/blank.pug`; would use Toasts §4.2 + Badges §3.6 once scoped | `ToastHost` | SPEC-0008 (notifications) | NotificationsEndpointTests, AreaTests | PARTIAL — page placeholder; API exists, no page model |
...
| — | — | — | — | — | static | `wwwroot/css/bootstrap-icons.css` | icon source | Bootstrap Icons | CoreUI Icons §8 (`cil-*`/`cif-*` SVGs → `wwwroot/icons`) | `IconSet` | — | — | RETAINED (icons in use; no CoreUI vendored asset required) |
